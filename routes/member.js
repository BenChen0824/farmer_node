...
router.post('/signup', async(req,res)=>{
    ...
    const sql01 = "INSERT INTO `customer_data`(`name`, `email`, `password`, `creat_at`) VALUES (?,?,?,NOW())"
...
router.put('/data', async (req,res)=>{
    ...
    const sql03 = 'UPDATE customer_data SET name=?, mobile=?, email=?, birthday=?, address=?, account=?,password=? WHERE customer_data.customer_id=?';

    const {name, mobile, email, birthday, address, account, password, customer_id} = req.body;
    const pass_hash = bcrypt.hashSync(`${password}`, 10)
    const [result] = await db.query(sql03,[name, mobile, email, birthday, address, account, pass_hash, customer_id]);
